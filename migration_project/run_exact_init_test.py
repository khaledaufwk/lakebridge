#!/usr/bin/env python3
"""Test exact same initialization as main notebook."""
import sys
import base64
import time
...
def main():
    ...
    notebook_local = Path(__file__).parent / "pipelines/timescaledb/notebooks/test_step_by_step.py"
    notebook_path = "/Workspace/migration_project/pipelines/timescaledb/notebooks/test_step_by_step"
    ...
    # Submit with same parameters as main notebook
    parameters = {
        "load_mode": "full",
        "run_optimize": "yes",
    ...
    print(f"\nSubmitting with parameters: {parameters}")
    run = w.jobs.submit(
        run_name="Test Exact Init",
        tasks=[
            SubmitTask(
                task_key="test",
