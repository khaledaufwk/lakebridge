#!/usr/bin/env python3
"""Deploy and run test notebook to diagnose issues."""
import sys
import base64
import time
...
def main():
    ...
    notebook_local = Path(__file__).parent / "pipelines/timescaledb/notebooks/test_connection.py"
    notebook_path = "/Workspace/migration_project/pipelines/timescaledb/notebooks/test_connection"

    print(f"Deploying test notebook...")
    ...
    # Submit notebook
    print("\nSubmitting test notebook...")
    run = w.jobs.submit(
        run_name="Test Connection",
        tasks=[
            SubmitTask(
                task_key="test",
    ...
    print(f"URL: {creds['databricks']['host']}/?o=3022397433351638#job/0/run/{run_id}")
