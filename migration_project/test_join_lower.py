"""Test Project join with case-insensitive comparison."""
import sys
...
print("=== Test Case-Insensitive Join ===")
...
joined = source.alias("s").join(
    ...
    F.lower(F.col("s.ProjectId")) == F.lower(F.col("p.ExtProjectID")),
    "inner"
)
...
if count > 0:
    print("\\n=== SUCCESS - Join works! ===")
    print("\\nSample:")
    joined.select("s.ProjectId", "p.ExtProjectID", "p.ProjectID", "p.ProjectName").limit(5).show(truncate=False)
else:
    print("\\n=== No matches even with case-insensitive! ===")
'''
